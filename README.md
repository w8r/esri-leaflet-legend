    ...
    if(!error) {
        ...
        for(var i = 0, len = legend.layers.length; i < len; i++) {
            ...
            for(var j = 0, jj = legend.layers[i].legend.length; j < jj; j++){
                html += L.Util.template('<li><img alt="{label}" width="{width}" height="{height}" src="data:{contentType};base64,{imageData}"><span>{label}</span></li>', legend.layers[i].legend[j]);
            }
            html += '</ul></li>';
        }
        html+='</ul>';
...
```js
{
    ...
    listRowTemplate: '<li><img alt="{label}" width="{width}" \
                               height="{height}" \
